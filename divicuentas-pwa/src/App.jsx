...
import { Receipt, Plus, Trash2, Users, Download, Upload, Wallet, Calculator, CheckCircle2, Share2 } from "lucide-react";
...
const STORAGE_KEY = "diviCuentas:v1";
...
export default function App() {
  ...
  const { people, items, currency, taxPct, tipPct, otherFees, paidBy, notes } = data;
  ...
  const exportJson = () => {
    ...
    a.href = url;
    a.download = `divicuentas-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  ...
  return (
    <div className="container">
      <div className="header">
        <div>
          <div className="h1"><Receipt size={28}/> DiviCuentas</div>
            ...
            <div>
              ...
              <div className="list">
                {settlements.map((s, idx) => {
                  ...
                  return <div key={idx} className="transfer"><span>{from} → {to}</span><span className="badge">{fmt(s.amount, currency)}</span></div>;
                })}
              </div>
            </div>
          )}
