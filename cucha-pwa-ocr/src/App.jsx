...
const fmt = (n, currency) =>
  new Intl.NumberFormat(undefined, { style: "currency", currency }).format(isFinite(n) ? n : 0);
...
export default function App() {
  ...
  // Scanner modal toggling
  const [showScanner, setShowScanner] = useState(false);
  const onScannerConfirm = (parsedItems) => {
    const newItems = parsedItems.map(it => ({ id: uid(), name: it.name, qty: it.qty, price: it.price, assigned: [] }));
    setData({ ...data, items: [...items, ...newItems] });
    setShowScanner(false);
  };

  // Fintoc helpers
  const canFintoc = currency === "CLP" && paidBy && (data.fintocUser || "").trim().length > 0;
  const fintocLink = (amount) => `https://fintoc.me/${encodeURIComponent(data.fintocUser)}/${Math.round(amount)}`;
  ...
  return (
    <div className="container">
            ...
            <div>
                ...
                {settlements.map((s, idx) => {
                  ...
                  return (
                    <div key={idx} className="transfer">
                      <span>{from} → {to}</span>
                      <span className="badge">{fmt(amount, currency)}</span>
                      {canFintoc && (
                        <a className="btn" href={fintocLink(amount)} target="_blank" rel="noopener">
                          <ExternalLink size={14}/> Transferir
                        </a>
                      )}
                    </div>
                  );
              ...
              {!canFintoc && paidBy && (
                <div className="small" style={{marginTop:8}}>
                  Para links de pago Fintoc, usa moneda CLP y completa el usuario Fintoc del pagador.
                </div>
              )}
            </div>
          )}
      ...
      <div className="footer">
        Hecho con 💙 para dividir cuentas. Tus datos se guardan localmente en este navegador.
      </div>
    </div>
  );
}
