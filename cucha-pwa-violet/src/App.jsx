...
import { Receipt, Plus, Trash2, Users, Download, Upload, Wallet, Calculator, CheckCircle2, Share2, Copy, ExternalLink, Send, Sparkles } from "lucide-react";
...
const DEFAULT_STATE = {
  ...
  items: [{ id: uid(), name: "Pizza", qty: 1, price: 12000, assigned: [] }, { id: uid(), name: "Bebidas", qty: 2, price: 2500, assigned: [] }],
  taxPct: 0, tipPct: 10, otherFees: 0, paidBy: "", notes: "", fintocUser: "", rounding: "none"
};
const STORAGE_KEY = "cucha:brand:v1";
...
function roundAmount(x, policy) {
  const n = Math.round(x);
  if (policy === "10") return Math.round(n / 10) * 10;
  if (policy === "100") return Math.round(n / 100) * 100;
  return n;
}
...
export default function App() {
  ...
  useEffect(() => { const handler = (e)=>{ e.preventDefault(); setDeferredPrompt(e); setInstallAvailable(true); }; window.addEventListener("beforeinstallprompt", handler); return ()=>window.removeEventListener("beforeinstallprompt", handler); }, []);
  ...
  const { people, items, currency, taxPct, tipPct, otherFees, paidBy, notes, fintocUser, rounding } = data;
  ...
  const perPersonTotals = useMemo(() => {
    ...
    people.forEach(p => {
      ...
      const prop = share / sumPreFees;
      const extra = total - subtotal;
      res[p.id] = share + extra * prop;
    });
    return res;
  ...
  const exportJson = ()=>{ try{ const filename = `cucha-${new Date().toISOString().slice(0,10)}.json`; const blob = new Blob([JSON.stringify(data,null,2)], {type: "application/json"}); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = filename; a.rel="noopener"; a.style.display="none"; document.body.appendChild(a); a.click(); setTimeout(()=>{document.body.removeChild(a); URL.revokeObjectURL(url);},0);}catch(e){ const dataStr = "data:application/json;charset=utf-8," + encodeURIComponent(JSON.stringify(data,null,2)); window.open(dataStr, "_blank"); } };
  const importJson = (file)=>{ const reader = new FileReader(); reader.onload = ()=>{ try{ const obj = JSON.parse(reader.result); setData({...DEFAULT_STATE, ...obj}); } catch { alert("Archivo inválido"); } }; reader.readAsText(file); };

  // Share helpers
  const payer = people.find(p => p.id === paidBy);
  const canFintoc = currency === "CLP" && paidBy && (fintocUser||"").trim().length>0;
  const roundAmount = (x) => {{/* defined above but capture policy */}};
  const roundWithPolicy = (x, policy) => {{/* not used here */}};
  const roundingPolicy = rounding;
  const doRound = (x) => {{/* wrapper */}};
  const roundFn = (x) => {{ return Math.round(x); }};

  const buildFintoc = (amount) => `https://fintoc.me/${encodeURIComponent(fintocUser)}/${(rounding==="10" ? Math.round(Math.round(amount)/10)*10 : (rounding==="100" ? Math.round(Math.round(amount)/100)*100 : Math.round(amount)))}`;
  ...
  const waMsg = (fromName, amount) => {
    const amt = (rounding==="10" ? Math.round(Math.round(amount)/10)*10 : (rounding==="100" ? Math.round(Math.round(amount)/100)*100 : Math.round(amount)));
  ...
  return (
    <div className="container">
      <div className="header">
        <div>
          <div className="h1"><Receipt size={28}/> Cucha <span className="brand-pill"><Sparkles size={14}/>violeta</span></div>
      ...
      <div className="row">
        <div className="card">
          ...
          <div className="list">
            ...
            <div style={{marginTop:8}}>
              <div className="small">Redondeo sugerido para transferencias</div>
              <select className="input" value={rounding} onChange={(e)=>setData({...data, rounding: e.target.value})}>
                <option value="none">Sin redondeo</option>
                <option value="10">A múltiplos de 10</option>
                <option value="100">A múltiplos de 100</option>
              </select>
            </div>
          </div>
        </div>
                  ...
                  {people.map((p) => (
                    <label key={p.id} className="flex">
                      <input className="checkbox" type="checkbox" checked={(it.assigned || []).includes(p.id)}
                        onChange={(e)=>{ const set = new Set(it.assigned || []); e.target.checked ? set.add(p.id) : set.delete(p.id); updateItem(it.id, { assigned: Array.from(set) }); }}/>
                      <span>{p.name}</span>
                    </label>
                  ))}
                </div>
        ...
        <div className="card" style={{gridColumn: "span 2"}}>
          ...
          <div className="flex" style={{flexWrap:'wrap'}}>
            ...
            <div style={{flex:'1 1 200px'}}>
              ...
              <div className="input" style={{display:'flex', alignItems:'center', fontWeight:700}}>{fmt(subtotal, currency)}</div>
            </div>
          </div>
          ...
          <div className="flex" style={{flexWrap:'wrap'}}>
            ...
            <div style={{flex:'1 1 200px'}}><div className="small">Total</div><div className="input" style={{fontWeight:800}}>{fmt(total, currency)}</div></div>
          </div>
            ...
            <div>
              ...
              <div className="list">
                {settlements.map((s, idx) => {
                  ...
                  const payer = people.find((p)=>p.id===s.to)?.name || "Pagador";
                  const raw = s.amount;
                  const rounded = (rounding==="10" ? Math.round(Math.round(raw)/10)*10 : (rounding==="100" ? Math.round(Math.round(raw)/100)*100 : Math.round(raw)));
                  const canFintoc = currency==="CLP" && paidBy && (fintocUser||"").trim().length>0;
                  const link = canFintoc ? `https://fintoc.me/${encodeURIComponent(fintocUser)}/${rounded}` : null;
                  return (
                    <div key={idx} className="transfer">
                      <span>{from} → {payer}</span>
                      <span className="badge">{fmt(rounded, currency)}</span>
                      <div className="flex">
                        {link && <a className="btn primary" href={link} target="_blank" rel="noopener">Transferir</a>}
                        {link && <button className="btn" onClick={()=>navigator.clipboard.writeText(link)}><Copy size={14}/> Copiar</button>}
                        <a className="btn" href={`https://wa.me/?text=${encodeURIComponent(`Hola ${from}! Tu parte es ${fmt(rounded, currency)}.${canFintoc?`\nPaga acá: ${link}`:''}`)}`} target="_blank" rel="noopener"><Send size={14}/> WhatsApp</a>
                      </div>
                    </div>
                  );
                })}
              </div>
              {paidBy && (!(currency==="CLP" && (fintocUser||"").trim().length>0)) && (
                <div className="small" style={{marginTop:8}}>Para links Fintoc usa **CLP** y completa el usuario Fintoc del pagador.</div>
              )}
            </div>
          )}
        </div>
      ...
      <div className="footer">Hecho con 💜 por Cucha. Tus datos se guardan localmente.</div>
    </div>
  );
}
